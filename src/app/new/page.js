...
const page = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (models.llama && activeModels.llama) llamaChat.sendMessage({ text: prompt });
        if (models.deepseek && activeModels.deepseek) deepseekChat.sendMessage({ text: prompt });
        ...
        <div className="relative w-full min-h-[calc(100vh-16px)] border rounded p-2 pt-2 flex flex-col gap-4 overflow-auto">
            <div className="flex gap-1 overflow-auto custom-scrollbar pb-1">
                ...
                {models.gemini && (
                    <ResponsePanel
                        model="Gemini"
                        modelIcon={modelIcons.gemini}
                        modelMessages={geminiChat.messages}
                        onToggle={() =>
                            setActiveModels((prev) => ({
                                ...prev,
                                gemini: !prev.gemini,
                ...
                {models.deepseek && (
                    ...
                    modelMessages={deepseekChat.messages}
                    onToggle={() =>
                        setActiveModels((prev) => ({
                            ...prev,
